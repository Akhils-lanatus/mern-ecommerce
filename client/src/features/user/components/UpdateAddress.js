...
const UpdateAddress = () => {
  ...
  const handleEditAddress = () => {
    dispatch(updateUserAddressAsync(formValues));
    showToast("SUCCESS", "Address Updated");
    navigate("/profile");
  };
  if (selectedAddress?.length === 0) return <Navigate to="/" />;
  return (
    <>
      ...
      {open && (
        <CustomDialog
          ...
          buttonColor="green"
          buttonText="Update"
          dialogContent="Are you sure you want to update selected address?"
